...
import { NFXLoader } from '../loader/NFXLoader';
const canvas = document.getElementById('nfx-canvas');
if (!canvas) {
...
renderer.onBeforeRender((dt) => {
    ...
    camera.position.set(Math.sin(orbit.yaw) * cp * orbit.distance, Math.sin(orbit.pitch) * orbit.distance + 0.2, Math.cos(orbit.yaw) * cp * orbit.distance);
